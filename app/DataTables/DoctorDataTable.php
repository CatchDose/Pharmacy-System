        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function (User $doctor){
                return $this->toggleBan($doctor->id) . '
                    <a class="btn btn-success rounded mx-1" id="option_a1" href="' .route("doctors.edit",$doctor->id).'" title="edit"> <i class="bi bi-pencil-square"></i> </a>
                    <form method="post" class="delete_item"  id="option_a3" action="' .route("doctors.destroy",$doctor->id).'">
                        <input type="hidden" name="_token" value="' .csrf_token(). '">
                        <input type="hidden" name="_method" value="DELETE">
                        <button type="submit " class="btn btn-danger" onclick="modalShow(event)" id="delete_ ' . $doctor->id . '" data-bs-toggle="modal" data-bs-target="#exampleModal" title="delete"><i class="bi bi-trash3"></i></button>
                    </form>
                </div>';
            })
